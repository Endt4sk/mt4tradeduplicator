...
void processTrades()
{
    ...
    for (i=0; i<k; i++)
    {
        ...
        if (in<0)
        {
            ...
            if (GetOriginalOrderTicket(StoredOrderTicket[i]) < 0) 
            {
        ...
        else
        {
                    ...
                    ||  (MathAbs(StoredOrdeTakeProfit[i]-g_StoredOrdeTakeProfit[in])>=p))
            {
                ...
                if (modifyOrder > 0)
                {
                    if (OrderSelect(modifyOrder, SELECT_BY_TICKET, MODE_TRADES))
                        if (( OrderMagicNumber() == g_StoredOrderTicket[in] ))
                        {
                            if (OrderType() == OP_BUY || OrderType() == OP_SELL)
                                BetterOrderModify(OrderSymbol(),  OrderType(), OrderTicket(), OrderOpenPrice(), StoredOrderStopLoss[i],
                                            StoredOrdeTakeProfit[i], 0,Blue);

                            else
                                BetterOrderModify(OrderSymbol(),  OrderType(), OrderTicket(), StoredOrderOpenPrice[i], StoredOrderStopLoss[i],
                                            StoredOrdeTakeProfit[i], 0,Blue);
            ...
            if(StoredOrderLots[i] < g_StoredOrderLots[in])
            {
                ...
                Print("partialCloseOrder: ", partialCloseOrder);
...
void SendCloseOrder(int orderTicket, double orderLots, double origLots, string orderSymbol)
{
            ...
            if (closeOrder > 0)
            {
 ...
 
                {
                    ...
                    OrderSelect(closeOrder, SELECT_BY_TICKET, MODE_TRADES);
                    double closeVolume = orderTotalLots * lotMultiplier;
                    if (closeVolume > OrderLots())
